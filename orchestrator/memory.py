...
async def store_session(messages: list[dict], session_id: str) -> None:
    ...
    # Title: empty initially — generated by _generate_session_title below or
    # by write_session_title() at session close
    title = ""
    ...
    if conversation_text:
        ...
        # Generate a descriptive session title — best-effort.
        # Runs after Graphiti regardless of whether episode storage succeeded.
        gen_title = await _generate_session_title(conversation_text, cfg)
        if gen_title:
            store.update_session_title_sync(session_id, gen_title)
            log.debug("memory: session title set -> %r", gen_title)
